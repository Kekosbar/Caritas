...
$nome = $login = $data = $cargo = $telefone = $email = $foto = $imagem = $descricao = $senha = $confsenha = $senhaAntiga = $senhaDoBD = "";
$nomeErr = $loginErr = $dataErr = $cargoErr = $telefoneErr = $emailErr = $fotoErr = $imagemErr = $descricaoErr = $senhaErr = $confsenhaErr = $senhaAntigaErr ="";

$id;
if(empty($_GET["idAdmin"])){
    sair();
}else{$id = $_GET["idAdmin"];}


require_once '../../../../php/classes/BDconnect.php';
$bd = new BDconnect();
$result = $bd->getUserAdminPorID($id);
if($result == null){
    sair();
}else{
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($_POST["senhaAntiga"])) {$senhaAntigaErr = "ERRO! Informe a senha antiga"; $validade = false;}
    else $senhaAntiga = $_POST["senhaAntiga"];
    
    if(md5($senhaAntiga) != $senhaDoBD){
        $senhaAntigaErr = "ERRO! Senha incorreta"; $validade = false;
//         echo "<script> alert('ERRO 1'); </script>";
    ...
    if($validade){
        ...
        $admin = new UserAdmin($id, $nome, $login."@login", $data, $cargo, $telefone, $email, $imagem, $descricao, md5($senha));
        $resultado = $bd->editarUserAdmin($admin);
        if($resultado){
            echo "<script> alert('Salvo com sucesso!'); window.location = './administrador.php'; </script>";
        }else{
            echo "<script> alert('Ocorreu um erro ao cadastrar os dados'); </script>";
        }
    }else{
        //echo "<script>alert('Erro no formulário');</script>";
        //$login = "";
    }
}
    ...
    <div id="wrapper">
      ...
      <div id="content-wrapper">
        ...
        <div class="container-fluid">
          ...
          <form method="post" action="<?php  echo htmlspecialchars($_SERVER["PHP_SELF"]).'?idAdmin='.$_GET["idAdmin"]; ?>" enctype="multipart/form-data">
              ...
              <div class="form-group">
                  <label>Login</label><span <?php if($loginErr != "") {echo 'class="erro">'; echo "$loginErr";} else echo '>';?></span><br>
                  <input class="form-control" type="text" name="login" value="<?php echo "$login" ?>" style="width: 200px; display: inline-block">
                  <span style="padding: 5px; padding-right: 10px; padding-left: 10px; font-weight: bold; background-color: #d5d5d5;  display: inline-block">@login</span>
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                  <label>Senha Antiga</label><span <?php if($senhaErr != "") {echo 'class="erro">'; echo "$senhaErr";} else echo '>';?></span>
                  <input class="form-control" name="senhaAntiga" type="password">
              </div>
              <div class="form-group">
                  <label>Nova Senha</label><span <?php if($senhaErr != "") {echo 'class="erro">'; echo "$senhaErr";} else echo '>';?></span>
        ...
        <footer class="sticky-footer">
          <div class="container my-auto">
            <div class="copyright text-center my-auto">
              <span>Copyright © Caritas Arquidiocesana de Diamantina 2018</span>
            </div>
          </div>
        </footer>
